class Stock < ApplicationRecord
    ...
    @hash.each do |data|
      ...
      if stock
        ...
        if prev_stock.present?
          ...
          new_stocks << Stock.new(material_id:material_id,date:date,used_amount:used_amount,end_day_stock:end_day_stock,start_day_stock:prev_stock.end_day_stock)
        else
          end_day_stock = 0 - used_amount
          new_stocks << Stock.new(material_id:material_id,date:date,used_amount:used_amount,end_day_stock:end_day_stock)
        end
        change_stock(update_stocks,material_id,date,end_day_stock)
      end
    end
  ...
  def self.initialize_stocks(date)
