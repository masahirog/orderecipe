class ProductSalesPotential < ApplicationRecord
  ...
  def self.recalculate_potential(store_id,product_id)
    ...
    if analysis_products.map{|ap|ap.potential}.sum > 0 && analysis_products.count > 0
      potential_average = (analysis_products.map{|ap|ap.potential}.sum/analysis_products.count).round
    else
      potential_average = 0
    end
