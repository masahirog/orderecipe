...
class StoreDailyMenusController < AdminController
  ...
  def input_manufacturing_number
    ...
    @analyses.map do |analysis|
      ...
      salad = "#{analysis_products_hash[analysis.id][22][:sales_number]}（#{analysis_products_hash[analysis.id][22][:manufacturing_number]}）"
      ...
      [weather,transaction_count,soup,fukusai,syusai,salad,curry,bento,sweets]
    end
    @tbody_contents = {}
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.csv do
        send_data render_to_string, filename: "#{date}_#{params[:store_id]}_shukei.csv", type: :csv
      end
    end
  end
  def ikkatsu
