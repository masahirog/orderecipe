class MasuOrderLoadingSheetPdf < Prawn::Document
  ...
  def table_content(products_num_h,date)
    ...
    bounding_box([20, 550], :width => 740) do
      table line_item_rows2(products_num_h,date,masu_orders,total) do
        ...
        self.header = true
        columns = Array.new(masu_orders.length){50}
  ...
  def line_item_rows2(products_num_h,date,masu_orders,total)
    ...
    masu_orders.each do |mo|
      ...
      hash2.store(mo.id,[mo.number,mo.number,miso,miso,tea,'◯','◯',seikyusho,ryoshusho])
    end
    ...
    [['おしぼり','オシボリ'],['お箸','オハシ'],['みそ汁','ミソシル'],['紙コップ','カミコップ'],['お茶','オチャ'],['保冷剤','ホレイザイ'],['納品書','ノウヒンショ'],['請求書','セイキュウショ'],['領収書','リョウシュウショ']].each_with_index do |yoso,i|
      arr = [yoso[0],yoso[1]]
      masu_orders.each do |masu_order|
