class MasuOrderLoadingSheetPdf < Prawn::Document
  def initialize(date,masu_orders,masu_orders_num_h,products_num_h)
    ...
    table_content(date,masu_orders,masu_orders_num_h,products_num_h)
  end

  def table_content(date,masu_orders,masu_orders_num_h,products_num_h)
    bounding_box([0, 570], :width => 820) do
      text "発行時間：#{Time.now.strftime("%Y年 %m月 %d日　%H:%M")}",size:9,:align => :right
      table line_item_rows2(date,masu_orders,masu_orders_num_h,products_num_h) do
  ...
  def line_item_rows2(date,masu_orders,masu_orders_num_h,products_num_h)
