...
class WeeklyMenuA4 < Prawn::Document
  ...
  def ura_right(next_menus,store,from,to)
    ...
    fill_color '000000'
    text_box("ポイントサービス",at: [480,135], width: 110, height: 40,size:11)
    text_box("お買上げ100円で1ポイントたまるポイントカードをご用意しています！QRコードよりご登録いただけます",at: [480,120], width: 220, height: 40,size:9)
    ...
    fill_color '000000'
    text_box("予約お取り置き",at: [480,70], width: 180, height: 40,size:11)
    ...
    if store.id == 9 || store.id == 29
      text_box("TEL：#{store.phone}　営業時間： 11:00-20:30（無休）",at: [520,-2], width: 280, height: 40,size:8)
    else
  ...
  def left_table_new(daily_menu,daily_menu_details)
    ...
    [
      ...
      [[13,14],"<font size='12'>旬野菜のサラダ Salad</font>　- 季節の野菜を15品目使用したサラダ -"],
      ...
      left_sozai_height = 530 if index == 3
    ...
    self.line_width = 2
    line [395, left_sozai_height-20], [770, left_sozai_height-20]
    stroke_color 'd8d8d8'
    stroke
    self.line_width = 1
    ...
    text_box("お弁当は、ご飯なし（おかずのみ）の場合は商品価格より108円引きでご提供しております。",at: [445,left_sozai_height-15], width: 350, height: 40,size:8)
